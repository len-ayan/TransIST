...
class Lasot(BaseVideoDataset):
    ...
    # 首先读取groundtruth_rect.txt来确定序列的长度
        gt_file = os.path.join(seq_path, "groundtruth_rect.txt")
        with open(gt_file, 'r', newline='') as f:
            reader = list(csv.reader(f))
            sequence_length = len(reader)
    # 假设目标始终可见，没有遮挡或者出界情况
        target_visible = torch.ones(sequence_length, dtype=torch.uint8)
...
#         return target_visible
    ...
    def _get_sequence_path(self, seq_id):
        ...
        return os.path.join(self.root, "train", class_name + '-' + vid_id)
    ...
    def _get_frame_path(self, seq_path, frame_id):
        return os.path.join(seq_path, 'ngnmf', '{:04}.jpg'.format(frame_id+1))    # frames start from 1
